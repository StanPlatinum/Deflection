...
static sgx_status_t SGX_CDECL Enclave_ocall_sgx_puts1time(void* pms)
{
	ms_ocall_sgx_puts1time_t* ms = SGX_CAST(ms_ocall_sgx_puts1time_t*, pms);
	ms->ms_retval = ocall_sgx_puts1time(ms->ms_str);
...
static const struct {
	...
	39,
	{
		...
		(void*)Enclave_ocall_sgx_puts1time,
...
{
	sgx_status_t status;
	ms_ecall_receive_entrylabel_t ms;
	ms.ms_buffer = buffer;
	ms.ms_sz = sz;
